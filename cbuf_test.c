...
int test_cbuf_single_malloc();
int test_cbuf_steps();
int test_cbuf_steps_malloc();
int test_cbuf_threaded();
int test_cbuf_threaded_malloc();
...
int main()
{
	...
	Z_inf(0, "ELAPSED: %ld", start);
  	...
  	start = clock();
	Z_inf(0, "single thread, single step malloc");
	err_cnt += test_cbuf_single_malloc();
	start = clock() - start;
	Z_inf(0, "ELAPSED: %ld", start);
	...
	err_cnt += test_cbuf_steps_malloc();
	start = clock() - start;
	Z_inf(0, "ELAPSED: %ld", start);
...
*/
  	start = clock();
	Z_inf(0, "%d threads, stepped: %d", THREAD_CNT, STEP_SIZE);
	err_cnt += test_cbuf_threaded_malloc();
	start = clock() - start;
	Z_inf(0, "ELAPSED: %ld", start);
...
{
	int err_cnt = 0;
	cbuf_t *c = cbuf_create_malloc(OBJ_SZ, OBJ_CNT);
	Z_die_if(!c, "expecting malloc'ed buffer");
...
int test_cbuf_threaded_malloc()
{
	...
	*/
	...
	/* circ buf */
	cbuf_t *buf = cbuf_create_malloc(OBJ_SZ, OBJ_CNT);
	Z_die_if(!buf, "fail to alloc malloc");
